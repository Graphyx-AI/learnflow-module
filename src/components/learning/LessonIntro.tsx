...
export default function LessonIntro({ lesson, lessonNumber, onStart, onClose }: LessonIntroProps) {
  return (
    <div className="flex flex-col items-center justify-end min-h-screen bg-background relative">
      ...
      {/* Bottom sheet */}
      <div className="w-full max-w-[420px] bg-surface border border-border rounded-t-[28px] p-7 px-6 pb-10 relative z-10 animate-slide-up">
        {/* Drag handle */}
        <div className="w-10 h-1 bg-foreground/15 rounded-full mx-auto mb-5" />

        {/* Icon */}
        <div className="text-[44px] text-center mb-3.5">{lesson.icon}</div>

        {/* Title */}
        <h2 className="font-display text-[22px] font-bold text-center text-foreground mb-2">
        ...
        {/* Topics */}
        <div className="bg-foreground/[0.03] border border-border rounded-xl p-3.5 px-4 mb-5">
          <div className="text-[11px] font-extrabold uppercase tracking-wider text-muted-foreground mb-2.5">
            Você vai aprender
          </div>
        ...
        <button
          onClick={onStart}
          className="w-full rounded-[14px] py-[17px] text-base font-black text-primary-foreground tracking-wide mb-2.5 transition-all duration-150 hover:brightness-110 active:scale-[0.98]"
        ...
        <button
          onClick={onClose}
          className="w-full bg-transparent border-none text-sm text-muted-foreground cursor-pointer py-2"
        >
          Agora não
        </button>
      </div>
